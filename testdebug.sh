...
LOG_DIR="${EXPERIMENT_DIR}/train_${MODEL_SIZE}B_mbs${MBS}_bs${BS}_tp${TP}_pp${PP}_seq${SEQ_LENGTH}_optim_${OPTIMIZER}_iter${TOTAL_ITERS}/nocompile${NO_TORCH_COMPILE}_fa_${USE_FLASH_ATTN}_seqpara_${SEQ_PARALLEL}_contiparam_${CONTI_PARAMS}_norm_${NORM}_TE_FP16_${TE_FP16}/${TIME_STAMP}"
TRAIN_LOG="${LOG_DIR}/running.log"
mkdir -p $LOG_DIR
echo $TRAIN_LOG
...
echo "throughput per GPU: $(python3 mean_log_value.py tmp.txt)" |& tee -a $TRAIN_LOG
rm tmp.txt

# echo '============================================================================================================'
grep -Eo 'elapsed time per iteration [^|]*' $TRAIN_LOG | sed -E 's/.*elapsed time per iteration \(ms\): ([0-9\.]+).*/\1/' > tmp.txt
echo "elapsed time per iteration: $(python3 mean_log_value.py tmp.txt)" |& tee -a $TRAIN_LOG
...
echo "mem usages: $(python3 mean_log_value.py tmp.txt)" |& tee -a $TRAIN_LOG
rm tmp.txt
