...
def config_parser():
    ...
    parser.add_argument(
        "--vocab-size", "-v", type=int, default=51200, help="Size of the vocab"
    )
    parser.add_argument(
        "--hidden-size",
        "-hs",
    ...
    parser.add_argument("--moe", action="store_true", help="Whether our model is MoE")
    parser.add_argument(
        "--num-experts", "-e", type=int, default=128, help="Number of experts for MoE"
    )
    parser.add_argument(
        "--expert-interval", "-ei", type=int, default=2, help="Expert interval for MoE"
    )
    parser.add_argument(
        "--topk", "-t", type=int, default=1, help="Top k routing for MoE"
    )
    parser.add_argument(
        "--swiglu",
        action="store_true",
...
def calc_params(args):
    ...
    assert (
        args.topk <= args.num_experts
    ), "You cannot route to more experts than you have!"
    assert (
        args.num_layers % args.expert_interval == 0
    ), "Require for simplicity that we don't have hanging dense layers"
