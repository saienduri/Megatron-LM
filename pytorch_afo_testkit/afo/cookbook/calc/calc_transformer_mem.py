...
def config_parser():
    ...
    parser.add_argument(
        "--num-gpus", type=int, default=1, help="Number of GPUs used for training"
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--batch-size-per-gpu", "-b", type=int, default=1, help="Batch size per GPU"
    )
    parser.add_argument(
        "--sequence-length",
        "-s",
    ...
    # Inference settings
    parser.add_argument(
        "--infer", action="store_true", help="whether we're doing inference"
    )
    parser.add_argument(
        "--kv-size-ratio",
        "-kv",
    ...
    # MoE Settings
    parser.add_argument("--num-experts", type=int, default=0, help="Number of experts")
    parser.add_argument(
        "--expert-parallelism",
        "-ep",
        type=int,
...
def calc_mem(args):
    ...
    mlp_params = (
        ...
        * args.num_layers
        * args.hidden_size
        * args.ffn_expansion_factor
        * args.hidden_size
    )
    total_params = (
        embed_params + positional_params + ln_params + attention_params + mlp_params
    )
    ...
    if args.num_experts > 0:
        total_moe_params = (
            ...
            + ln_params
            + attention_params
            + (args.num_experts * mlp_params)
        )
    # Split the model with 3D parallelism
    if args.num_experts == 0:
        per_gpu_model_mem = (total_params * bytes_per_param) / (
            args.tensor_parallel_size * args.pipeline_parallel_size
        )
    else:
        EP_total_params = (
            ...
            + ln_params
            + attention_params
            + ((args.num_experts / args.expert_parallelism) * mlp_params)
        )
        per_gpu_model_mem = (EP_total_params * bytes_per_param) / (
            args.tensor_parallel_size * args.pipeline_parallel_size
        )
    ...
    if not args.infer and args.checkpoint_activations:
        activation_mem = (
            ...
            * args.hidden_size
            * args.num_layers
            * ((16 * args.low_prec_bytes_per_val + 2))
        )
        ...
        activation_mem = (
            ...
            * (
                (16 * args.low_prec_bytes_per_val + 2)
                + (2 * args.low_prec_bytes_per_val + 1)
                * (args.num_attention_heads * args.sequence_length / args.hidden_size)
            )
        )
    ...
    if args.infer:
        per_gpu_mem_gib = (
            per_gpu_activation_mem_gib
            + per_gpu_kv_cache_mem_gib
            + per_gpu_model_mem_gib
            + args.misc_mem_gib
        )
        single_replica_mem_gib = (
            activation_mem_gib
            + kv_cache_mem_gib
            + model_mem_gib
            + args.misc_mem_gib * args.num_gpus
        )
    else:
        per_gpu_mem_gib = (
            per_gpu_activation_mem_gib
            + per_gpu_gradient_mem_gib
            + per_gpu_model_mem_gib
            + per_gpu_optimizer_mem_gib
            + per_gpu_communication_mem_gib
            + args.misc_mem_gib
        )
        single_replica_mem_gib = (
            activation_mem_gib
    ...
    print("*** Total GPU Memory for a Single Model Replica")
