...
def config_parser():
    ...
    parser.add_argument(
        "--num-experts", "-e", type=int, default=8, help="Number of experts for MoE"
    )
    parser.add_argument(
        "--expert-interval", "-ei", type=int, default=1, help="Expert interval for MoE"
    )
    parser.add_argument(
...
def calc_params(args):
    ...
    if args.moe:
        ...
        ffn_dense_params = (
            2
            * args.ffn_expansion_factor
            * (args.num_layers - num_expert_layers)
            * args.hidden_size
            * args.hidden_size
        )
    ...
    print(
        f"Calculating number of parameters with training configuration: {vars(args)}\n"
    )
