...
def run_all_to_all(local_rank, args):
    ...
    if args.single:
        ...
        try:
            ...
            assert (
                mat.numel() % world_size == 0
            ), f"tensor cannot be divided in {world_size} chunks"
            sync_all()
        ...
        try:
            ...
            assert (
                mat.numel() % world_size == 0
            ), f"tensor with {mat.numel()} elements cannot be divided in {world_size} chunks"
            input = (mat.mul_(global_rank)).view(-1)
            # Delete original mat to avoid OOM
            del mat
            torch.cuda.empty_cache()
            output = torch.zeros(
                elements_per_gpu, dtype=getattr(torch, args.dtype)
            ).cuda(local_rank)
        except RuntimeError as e:
            if "out of memory" in str(e):
                if dist.get_rank() == 0:
