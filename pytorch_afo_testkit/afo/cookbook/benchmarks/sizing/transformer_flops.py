...
def benchmark_transformer_from_mm_and_bmm(
    args,
    configuration,
    seq_length,
    global_batch_size,
    ...
    if "dropout" in args.blocks or "all" in args.blocks:
        elapsed_attention_time += benchmark_dropout(
            (
                microbatch_size,
                num_attention_heads // tensor_mp_size,
                seq_length,
                seq_length,
            ),
            "attention_dropout",
    ...
    elapsed_total_time = (
        elapsed_attention_time
        + elapsed_mlp_time
        + elapsed_add_bias_dropout_time
    ...
    if elapsed_attention_time > 0:
        attention_throughput = num_attention_floating_point_operations / (
            elapsed_attention_time * 10**12
        )
    if elapsed_mlp_time > 0:
        mlp_throughput = num_mlp_floating_point_operations / (elapsed_mlp_time * 10**12)
    if elapsed_total_time > 0:
        total_throughput = num_total_floating_point_operations / (
            elapsed_total_time * 10**12
        )

    print()
    for elapsed_time, throughput, label in zip(
        [elapsed_attention_time, elapsed_mlp_time, elapsed_total_time],
    ...
    print(
        "Transformer - MLP - Attention (in seconds): "
...
def benchmark_transformer(
    ...
    mlp_layer = (
        ParallelMLP(args, init_method=init_method, output_layer_init_method=init_method)
        .half()
        .to("cuda")
    )
    transformer_layer = (
    ...
    inp = (
        torch.randn((args.seq_length, args.batch_size, args.hidden_size))
        .half()
        .to("cuda")
    )
    attention_mask = torch.tril(
        torch.ones((1, args.seq_length, args.seq_length), device="cuda")
    ...
    for layer, label, need_attention_mask, num_floating_point_operations in zip(
        [attention_layer, mlp_layer, transformer_layer],
        ["Attention", "MLP", "Transformer"],
        [True, False, True],
        [
            num_attention_floating_point_operations,
...
if __name__ == "__main__":
    ...
    h_group.add_argument(
        ...
        nargs="+",
        type=int,
        help="The hidden dimension, enter any number of arguments",
    )
    h_group.add_argument(
        "--hidden_size_range",
        nargs="+",
        type=int,
        help="The hidden dimension, [start,stop,step]",
    )
    ...
    a_group.add_argument(
        ...
        nargs="+",
        type=int,
        help="The number of attention heads, enter any number of arguments",
    )
    a_group.add_argument(
        "--num_attention_heads_range",
        nargs="+",
        type=int,
        help="The number of attention heads, [start,stop,step]",
    )
    ...
    v_group.add_argument(
        ...
        nargs="+",
        type=int,
        help="The vocabulary size, enter any number of arguments",
    )
    v_group.add_argument(
        "--vocab_size_range",
        nargs="+",
        type=int,
        help="The vocabulary size, [start,stop,step]",
    )
    ...
    s_group.add_argument(
        ...
        nargs="+",
        type=int,
        help="The sequence length, enter any number of arguments",
    )
    s_group.add_argument(
        "--seq_length_range",
        nargs="+",
        type=int,
        help="The sequence length, [start,stop,step]",
    )
    ...
    b_group.add_argument(
        ...
        nargs="+",
        type=int,
        help="The microbatch size, enter any number of arguments",
    )
    b_group.add_argument(
        "--microbatch_size_range",
        nargs="+",
        type=int,
        help="The microbatch size, [start,stop,step]",
    )
    ...
    gb_group.add_argument(
        ...
        nargs="+",
        type=int,
        help="The global batch size, enter any number of arguments",
    )
    gb_group.add_argument(
        "--global_batch_size_range",
        nargs="+",
        type=int,
        help="The global batch size, [start,stop,step]",
    )
    ...
    t_group.add_argument(
        ...
        nargs="+",
        type=int,
        help="The tensor parallel size, enter any number of arguments",
    )
    t_group.add_argument(
        "--tensor_mp_size_range",
        nargs="+",
        type=int,
        help="The tensor parallel size, [start,stop,step]",
    )
    ...
    parser.add_argument(
        ...
        nargs="+",
        type=str,
        help='The transformer blocks to benchmark, enter "all" or any number of [qkv_transform, attention_score, \
    ...
    parser.add_argument(
        ...
        type=int,
        default=200,
        help="The number of iterations used to benchmark each BMM",
    )
    parser.add_argument(
        "--num_warmup_iterations",
        type=int,
    ...
    for train_batch_size in global_batch_size:
        ...
        for configuration in configurations:
            ...
            if args.blocks is None:
                ...
                benchmark_transformer_from_mm_and_bmm(
                    args,
                    configuration,
                    seq_length,
                    train_batch_size,
