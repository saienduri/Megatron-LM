    ...
    with open(logfile_name, "r") as f:
        ...
        for line in f:
            ...
            if line == "Estimate":
                ...
                reading_estimate = False
            match = re.match(
                r"num_attention_heads: (\d+), hidden_size: (\d+), "
            ...
            match = re.match(
                r"Throughput \(in TFLOP/s\) for qkv_transform \((.*)\): (\d+\.\d+)",
                line,
            )
            if match is not None:
            ...
            match = re.match(
                r"Throughput \(in TFLOP/s\) for attention_score \((.*)\): (\d+\.\d+)",
                line,
            )
            if match is not None:
                throughput = float(match.group(2))
            ...
            match = re.match(
                r"Throughput \(in TFLOP/s\) for attention_over_value \((.*)\): (\d+\.\d+)",
                line,
            )
            if match is not None:
                throughput = float(match.group(2))
            ...
            match = re.match(
                r"Throughput \(in TFLOP/s\) for attention_dropout \((.*)\): (\d+\.\d+)",
                line,
            )
            if match is not None:
                throughput = float(match.group(2))
            ...
            match = re.match(
                r"Elapsed time for attention_softmax \((.*)\): (\d+\.\d+)", line
            )
            if match is not None:
                throughput = float(match.group(2))
                if reading_estimate:
            ...
            match = re.match(
                r"Throughput \(in TFLOP/s\) for attention_linear_projection \((.*)\): (\d+\.\d+)",
                line,
            )
            if match is not None:
                throughput = float(match.group(2))
            ...
            match = re.match(
                r"Throughput \(in TFLOP/s\) for mlp_h_to_4h \((.*)\): (\d+\.\d+)", line
            )
            if match is not None:
                throughput = float(match.group(2))
                if reading_estimate:
            ...
            match = re.match(
                r"Throughput \(in TFLOP/s\) for mlp_4h_to_h \((.*)\): (\d+\.\d+)", line
            )
            if match is not None:
                throughput = float(match.group(2))
                if reading_estimate:
            ...
            match = re.match(
                r"Elapsed time for mlp_fused_gelu \((.*)\): (\d+\.\d+)", line
            )
            if match is not None:
                throughput = float(match.group(2))
                if reading_estimate:
            ...
            match = re.match(
                r"Elapsed time for transformer_add_bias_dropout \((.*)\): (\d+\.\d+)",
                line,
            )
            if match is not None:
                throughput = float(match.group(2))
            ...
            match = re.match(
                r"Elapsed time for transformer_layer_norm \((.*)\): (\d+\.\d+)", line
            )
            if match is not None:
                throughput = float(match.group(2))
                if reading_estimate:
            ...
            match = re.match(
                r"Throughput \(in TFLOP/s\) for logit_block \((.*)\): (\d+\.\d+)", line
            )
            if match is not None:
                throughput = float(match.group(2))
                if reading_estimate:
...
def read_mm_logfile(logfile_name):
    ...
    with open(logfile_name, "r") as f:
        for line in f:
            line = line.strip()
            match = re.match(
                r"Throughput \(in TFLOP/s\) for (\d+)x(\d+)x(\d+): (\d+\.\d+)", line
            )
            if match is not None:
                m, n, k = int(match.group(1)), int(match.group(2)), int(match.group(3))
                throughput = float(match.group(4))
                throughputs.append({"m": m, "n": n, "k": k, "throughput": throughput})
    return throughputs
...
def read_bmm_logfile(logfile_name):
    ...
    with open(logfile_name, "r") as f:
        for line in f:
            line = line.strip()
            match = re.match(
                r"Throughput \(in TFLOP/s\) for bmm \((\d+)x(\d+)x(\d+)x(\d+)\): (\d+\.\d+)",
                line,
            )
            if match is not None:
                b, m, n, k = (
                ...
                throughputs.append(
                    {"b": b, "m": m, "n": n, "k": k, "throughput": throughput}
                )
    return throughputs
