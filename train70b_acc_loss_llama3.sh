...
DATA_DIR=../bookdata/
DATA_PATH=../bookdata/bookcorpus_text_sentence
DATA_PATH=$TRAIN_DATA
# /home/amd/guihong/bookdata/data/bookcorpus_text_sentence.bin
# Prepare the dataset
...
#   python3 tools/preprocess_data.py --input ${DATA_DIR}/bookcorpus_megatron.json  --tokenizer-type HuggingFaceTokenizer --tokenizer-model $TOKENIZER_MODEL --output-prefix ${DATA_DIR}/bookcorpus --workers `nproc` --split-sentences
#   python3 tools/preprocess_data.py --input ${DATA_DIR}/bookcorpus_megatron.json  --tokenizer-type HuggingFaceTokenizer --tokenizer-model $TOKENIZER_MODEL --output-prefix ${DATA_DIR}/bookcorpus --workers `nproc` --split-sentences
# else
...
DEFAULT_LOG_DIR="${EXPERIMENT_DIR}/${NNODES}nodes_rank${NODE_RANK}_train_${MODEL_SIZE}B_mbs${MBS}_bs${BS}_tp${TP}_pp${PP}_cp${CP}_iter${TOTAL_ITERS}/nocompile${NO_TORCH_COMPILE}_TE_FP16_${TE_FP16}/${TIME_STAMP}"
...
EXTRA_ARGS="$EXTRA_ARGS --torch-profiling"
fi
