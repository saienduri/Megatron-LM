...
TRAIN_LOG="${EXPERIMENT_DIR}/train_${MODEL_SIZE}B_iter${TOTAL_ITERS}_mbs${MBS}_bs${BS}_tp${TP}_pp${PP}_seq${SEQ_LENGTH}_optim_${OPTIMIZER}_nocompile${NO_TORCH_COMPILE}_fa_${USE_FLASH_ATTN}_seqpara_${SEQ_PARALLEL}_contiparam_${CONTI_PARAMS}_norm_${NORM}_TE_FP16_${TE_FP16}.log"
...
EXTRA_ARGS="$EXTRA_ARGS --torch-profiling --trace-dir $TRACE_DIR"
fi


# --use-flash-attn has been deprecated, which doesn't seem to affect model performance
