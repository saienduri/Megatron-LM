...
BASE_SRC_PATH=${BASE_SRC_PATH:-"/megatron-deepspeed"}
...
if [ "${USE_INTERNAL_DATA}" = "true" ]; then
...
elif [ "${DOWNLOAD_DATASET}" = "true" ]; then
    [ -d "${BASE_DATA_PATH}" ] || mkdir -p "${BASE_DATA_PATH}"

    DATA_BLEND="${BASE_DATA_PATH}/BookCorpusDataset_text_document"
    if ! [ -f "${DATA_BLEND}.bin"  ]; then
        wget -O "${DATA_BLEND}.bin" https://the-eye.eu/public/AI/pile_neox/data/BookCorpusDataset_text_document.bin
    fi
    if ! [ -f "${DATA_BLEND}.idx"  ]; then
        wget -O "${DATA_BLEND}.idx" https://the-eye.eu/public/AI/pile_neox/data/BookCorpusDataset_text_document.idx
    fi
    ...
    VOCAB_PATH="${BASE_DATA_PATH}/gpt2-vocab.json"
    MERGE_PATH="${BASE_DATA_PATH}/gpt2-merges.txt"
    DATA_BLEND="${BASE_DATA_PATH}/my-gpt2_text_document"
fi
###############################################################################
data_options=" \
...
template_json="${DIR}/ds_config_gpt_TEMPLATE.json"
config_json="${config_dir}/ds_config_gpt_zero${ZERO_STAGE}_${NAME}.json"
...
run_cmd="torchrun ${distributed_options} ${DIR}/../../pretrain_gpt.py ${megatron_options} ${data_options} ${deepspeed_options} 2>&1 | tee ${LOG_BASEPATH}/${NAME}_${host}_${current_time}.log"
echo ${run_cmd}
eval ${run_cmd}
set +x
