...
TRAIN_LOG="${LOG_DIR}/gemm_tune.log"
mkdir -p $LOG_DIR
echo $TRAIN_LOG
...
PROFILING_DIR="${LOG_DIR}/trace_tuning_gemm"
...
if [[ $NODE_RANK -eq $NUM_GROUPS ]]; then
    ...
    echo "Gemm_TUNE_$NNODES	$MODEL_SIZE	$SEQ_LENGTH	$MBS	$BS	$TP	$PP	$TGS	$PERFORMANCE	$MEMUSAGE	$ETPI" |& tee -a ../out.csv
    echo "Gemm_TUNE_$NNODES	$MODEL_SIZE	$SEQ_LENGTH	$MBS	$BS	$TP	$PP	$TGS	$PERFORMANCE	$MEMUSAGE	$ETPI" |& tee -a out.csv
else
