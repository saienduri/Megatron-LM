...
class ChatDataset(Dataset):
    ...
    def __init__(self, dataset_name, datapaths, tokenizer, max_seq_length, pad_to_max=True):
    ...
    def process_single_datapath(self, datapath):
        ...
        for jsonl in fin:
            ...
            if len(input_ids) > 0 and self.pad_to_max:
                max_seq_length = self.max_seq_length
                input_ids = input_ids[:max_seq_length].tolist()
                mask = mask[:max_seq_length].tolist()
                current_length = len(input_ids)
                input_ids.extend([self.tokenizer.eos_token_id for _ in range(
                    max(0, max_seq_length-current_length))])
                mask.extend([False for _ in range(
                    max(0, max_seq_length-current_length))])
