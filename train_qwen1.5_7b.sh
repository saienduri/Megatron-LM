...
EXPERIMENT_DIR="/workspace/pretrain/qwen1.5_7B"
...
if ! [ -f ${DATA_DIR}/bookcorpus_text_sentence.idx ]; then
  ...
  python3 tools/preprocess_data.py --input ${DATA_DIR}/bookcorpus_megatron.json  --tokenizer-type QWen2Tokenizer --tokenizer-model Qwen/Qwen1.5-7B --seq-length 2048 --extra-vocab-size 293 --output-prefix ${DATA_DIR}/bookcorpus --workers `nproc` --split-sentences
  python3 tools/preprocess_data.py --input ${DATA_DIR}/bookcorpus_megatron.json  --tokenizer-type QWen2Tokenizer --tokenizer-model Qwen/Qwen1.5-7B --seq-length 2048 --extra-vocab-size 293 --output-prefix ${DATA_DIR}/bookcorpus --workers `nproc` --split-sentences
fi
...
TRAIN_LOG="${EXPERIMENT_DIR}/logs/log_7B_${TP}_${PP}_${DP}_${MBS}_${GBS}_${SEQ_LENGTH}_${COMPILE}_${FA}_${DO}_${AC}.txt"
run_cmd="torchrun $DISTRIBUTED_ARGS \
        ...
        --tokenizer-model Qwen/Qwen1.5-7B \
