...
EXPERIMENT_DIR="experiment_lora"
mkdir -p $EXPERIMENT_DIR

DATA_PATH=../data/openhermes-2.5/openhermes2_5.jsonl
# DATA_PATH=../data/tiny_text/tiny-textbooks/train-00000-of-00001.parquet
...
if [[ $MODEL_SIZE -eq 8 ]]; then #llama3.1-8B
        ...
        NUM_LAYERS=126 # e.g. llama-13b: 40
...
if [[ $NODE_RANK -eq $NUM_GROUPS ]]; then
    'EXP_NAME	#GPUS	Model_SIZE 	Seq_Len	RANK	TE_FP8	MBS	GBS	TP	PP	CP	Tokens/Sec/GPU	TFLOPs/s/GPU	Memory Usage	Time/iter'
    echo "${EXP_NAME}	$WORLD_SIZE	$MODEL_SIZE	$SEQ_LENGTH	$RANK	$TE_FP8	$MBS	$BS	$TP	$PP	$CP	$TGS	$PERFORMANCE	$MEMUSAGE	$ETPI" |& tee -a ../out_lora.csv
    echo "${EXP_NAME}	$WORLD_SIZE	$MODEL_SIZE	$SEQ_LENGTH	$RANK	$TE_FP8	$MBS	$BS	$TP	$PP	$CP	$TGS	$PERFORMANCE	$MEMUSAGE	$ETPI" |& tee -a out_lora.csv
else
